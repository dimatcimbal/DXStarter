...
bool MaterialBuilder::CreateMaterial(Device& Device, std::shared_ptr<Material>& OutMaterial) {
    if (!mRootSigBytecode) {
        LOG_ERROR("Root signature bytecode not set.");
        return false;
    }

    if (!mVertexShaderBytecode) {
        LOG_ERROR("Vertex shader bytecode not set.");
        return false;
    }

    if (!mPixelShaderBytecode) {
        LOG_ERROR("Pixel shader bytecode not set.");
        return false;
    }

    std::unique_ptr<RootSignature> pRootSignature;
    if (!Device.CreateRootSignature(*mRootSigBytecode, pRootSignature)) {
    ...
    // Vertex Shader
    PSODesc.VS.pShaderBytecode = mVertexShaderBytecode->GetBuffer();
    PSODesc.VS.BytecodeLength = mVertexShaderBytecode->GetSize();
    ...
    // Pixel Shader
    PSODesc.PS.pShaderBytecode = mPixelShaderBytecode->GetBuffer();
    PSODesc.PS.BytecodeLength = mPixelShaderBytecode->GetSize();
