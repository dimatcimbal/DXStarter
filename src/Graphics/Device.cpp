...
bool Device::CreateMeshInstance(Mesh& Model, std::unique_ptr<MeshInstance>& Mesh) {
    size_t vertexDataSize = Model.GetVertexBufferSize();
    ...
    if (!CreateBuffer(L"MeshConstUploadBuffer", D3D12_HEAP_TYPE_UPLOAD,
                      D3D12_RESOURCE_STATE_GENERIC_READ, vertexDataSize, meshConstCpuBuffer)) {
        LOG_ERROR(L"Failed to create upload buffer.\n");
        return false;
    }

    // The buffer that holds Model's matrices
    std::unique_ptr<DeviceBuffer> meshConstDeviceBuffer;
    if (!CreateBuffer(L"MeshConstDeviceBuffer", D3D12_HEAP_TYPE_DEFAULT,
                      D3D12_RESOURCE_STATE_COMMON, vertexDataSize, meshConstDeviceBuffer)) {
        LOG_ERROR(L"Failed to create default buffer.\n");
        return false;
    }

    Mesh = std::make_unique<MeshInstance>(Model, std::move(meshConstCpuBuffer),
                                          std::move(meshConstDeviceBuffer));
    return true;
}
